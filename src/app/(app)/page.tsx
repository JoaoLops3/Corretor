  ...
  return (
    <section className="animate-fade-in">
      <div className="mb-3.5 flex flex-wrap items-baseline justify-between gap-2.5">
        ...
        <Link
          href="/imoveis?novo=1"
          className="inline-flex items-center gap-1.5 rounded-[10px] bg-ink px-4 py-2.5 text-[13px] font-semibold text-white shadow-[var(--shadow-sm-brand)] hover:bg-ink-2"
        >
      ...
      <div className="mb-5 grid grid-cols-2 gap-2.5 md:grid-cols-4">
        {data.stats.map((s, i) => (
          <Link
            key={s.label}
            href={s.href}
            className={`min-w-0 rounded-[14px] p-3.5 shadow-[var(--shadow-sm-brand)] transition-transform hover:-translate-y-0.5 md:p-4 ${statVariants[i].className}`}
          >
      ...
      <div className="mb-3.5 flex items-baseline justify-between">
        <div className="text-base font-bold">Próximas visitas</div>
        <Link href="/calendario" className="font-mono text-[11px] uppercase tracking-wider text-cyan">
          Ver agenda
        </Link>
      </div>
      {data.upcomingVisits.length === 0 ? (
        ...
        data.upcomingVisits.map((c) => (
          <div
            key={c.id}
            className={`mb-2.5 flex gap-3 rounded-[14px] border-y border-r border-line ${tipoBorderClass("visita")} bg-paper-2 p-3 shadow-[var(--shadow-sm-brand)]`}
          >
